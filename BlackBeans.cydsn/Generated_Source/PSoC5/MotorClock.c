...
/*******************************************************************************
* Function Name: MotorClock_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void MotorClock_Start(void) 
{
    /* Set the bit to enable the clock. */
    MotorClock_CLKEN |= MotorClock_CLKEN_MASK;
	MotorClock_CLKSTBY |= MotorClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: MotorClock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void MotorClock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    MotorClock_CLKEN &= (uint8)(~MotorClock_CLKEN_MASK);
	MotorClock_CLKSTBY &= (uint8)(~MotorClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: MotorClock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void MotorClock_StopBlock(void) 
{
    if ((MotorClock_CLKEN & MotorClock_CLKEN_MASK) != 0u)
    {
...
#if defined(MotorClock__CFG3)
        CLK_DIST_AMASK = MotorClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = MotorClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* MotorClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(MotorClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(MotorClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: MotorClock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void MotorClock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        MotorClock_CLKSTBY &= (uint8)(~MotorClock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: MotorClock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void MotorClock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = MotorClock_GetSourceRegister();
    uint16 oldDivider = MotorClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = MotorClock_CLKEN & MotorClock_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((MotorClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: MotorClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 MotorClock_GetDividerRegister(void) 
{
    return CY_GET_REG16(MotorClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: MotorClock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void MotorClock_SetModeRegister(uint8 modeBitMask) 
{
    MotorClock_MOD_SRC |= modeBitMask & (uint8)MotorClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: MotorClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void MotorClock_ClearModeRegister(uint8 modeBitMask) 
{
    MotorClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(MotorClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: MotorClock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 MotorClock_GetModeRegister(void) 
{
    return MotorClock_MOD_SRC & (uint8)(MotorClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: MotorClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void MotorClock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = MotorClock_GetDividerRegister();
    uint8 oldSrc = MotorClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        MotorClock_MOD_SRC |= CYCLK_SSS;
        MotorClock_MOD_SRC =
            (MotorClock_MOD_SRC & (uint8)(~MotorClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: MotorClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 MotorClock_GetSourceRegister(void) 
{
    return MotorClock_MOD_SRC & MotorClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: MotorClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void MotorClock_SetPhaseRegister(uint8 clkPhase) 
{
    MotorClock_PHASE = clkPhase & MotorClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: MotorClock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 MotorClock_GetPhaseRegister(void) 
{
    return MotorClock_PHASE & MotorClock_PHASE_MASK;
}
