...
/*******************************************************************************
* Function Name: BLDCClock_1_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BLDCClock_1_Start(void) 
{
    /* Set the bit to enable the clock. */
    BLDCClock_1_CLKEN |= BLDCClock_1_CLKEN_MASK;
	BLDCClock_1_CLKSTBY |= BLDCClock_1_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: BLDCClock_1_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BLDCClock_1_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    BLDCClock_1_CLKEN &= (uint8)(~BLDCClock_1_CLKEN_MASK);
	BLDCClock_1_CLKSTBY &= (uint8)(~BLDCClock_1_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: BLDCClock_1_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BLDCClock_1_StopBlock(void) 
{
    if ((BLDCClock_1_CLKEN & BLDCClock_1_CLKEN_MASK) != 0u)
    {
...
#if defined(BLDCClock_1__CFG3)
        CLK_DIST_AMASK = BLDCClock_1_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = BLDCClock_1_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* BLDCClock_1__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(BLDCClock_1_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(BLDCClock_1_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: BLDCClock_1_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BLDCClock_1_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        BLDCClock_1_CLKSTBY &= (uint8)(~BLDCClock_1_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: BLDCClock_1_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BLDCClock_1_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = BLDCClock_1_GetSourceRegister();
    uint16 oldDivider = BLDCClock_1_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = BLDCClock_1_CLKEN & BLDCClock_1_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((BLDCClock_1_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: BLDCClock_1_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 BLDCClock_1_GetDividerRegister(void) 
{
    return CY_GET_REG16(BLDCClock_1_DIV_PTR);
}


/*******************************************************************************
* Function Name: BLDCClock_1_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BLDCClock_1_SetModeRegister(uint8 modeBitMask) 
{
    BLDCClock_1_MOD_SRC |= modeBitMask & (uint8)BLDCClock_1_MODE_MASK;
}


/*******************************************************************************
* Function Name: BLDCClock_1_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BLDCClock_1_ClearModeRegister(uint8 modeBitMask) 
{
    BLDCClock_1_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(BLDCClock_1_MODE_MASK));
}


/*******************************************************************************
* Function Name: BLDCClock_1_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 BLDCClock_1_GetModeRegister(void) 
{
    return BLDCClock_1_MOD_SRC & (uint8)(BLDCClock_1_MODE_MASK);
}


/*******************************************************************************
* Function Name: BLDCClock_1_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BLDCClock_1_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = BLDCClock_1_GetDividerRegister();
    uint8 oldSrc = BLDCClock_1_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        BLDCClock_1_MOD_SRC |= CYCLK_SSS;
        BLDCClock_1_MOD_SRC =
            (BLDCClock_1_MOD_SRC & (uint8)(~BLDCClock_1_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: BLDCClock_1_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 BLDCClock_1_GetSourceRegister(void) 
{
    return BLDCClock_1_MOD_SRC & BLDCClock_1_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: BLDCClock_1_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void BLDCClock_1_SetPhaseRegister(uint8 clkPhase) 
{
    BLDCClock_1_PHASE = clkPhase & BLDCClock_1_PHASE_MASK;
}


/*******************************************************************************
* Function Name: BLDCClock_1_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 BLDCClock_1_GetPhaseRegister(void) 
{
    return BLDCClock_1_PHASE & BLDCClock_1_PHASE_MASK;
}
