/*******************************************************************************
* File Name: BLDCMotor_1_FastClock.c
* Version 2.20
*
*  Description:
...
#include "BLDCMotor_1_FastClock.h"
...
/*******************************************************************************
* Function Name: BLDCMotor_1_FastClock_Start
********************************************************************************
*
* Summary:
...
void BLDCMotor_1_FastClock_Start(void) 
{
    ...
    BLDCMotor_1_FastClock_CLKEN |= BLDCMotor_1_FastClock_CLKEN_MASK;
	BLDCMotor_1_FastClock_CLKSTBY |= BLDCMotor_1_FastClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: BLDCMotor_1_FastClock_Stop
********************************************************************************
*
* Summary:
...
void BLDCMotor_1_FastClock_Stop(void) 
{
    ...
    BLDCMotor_1_FastClock_CLKEN &= (uint8)(~BLDCMotor_1_FastClock_CLKEN_MASK);
	BLDCMotor_1_FastClock_CLKSTBY &= (uint8)(~BLDCMotor_1_FastClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: BLDCMotor_1_FastClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((BLDCMotor_1_FastClock_CLKEN & BLDCMotor_1_FastClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = BLDCMotor_1_FastClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = BLDCMotor_1_FastClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* BLDCMotor_1_FastClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(BLDCMotor_1_FastClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(BLDCMotor_1_FastClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: BLDCMotor_1_FastClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: BLDCMotor_1_FastClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = BLDCMotor_1_FastClock_GetSourceRegister();
    uint16 oldDivider = BLDCMotor_1_FastClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = BLDCMotor_1_FastClock_CLKEN & BLDCMotor_1_FastClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(BLDCMotor_1_FastClock_DIV_PTR, clkDivider);
                BLDCMotor_1_FastClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                BLDCMotor_1_FastClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(BLDCMotor_1_FastClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((BLDCMotor_1_FastClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(BLDCMotor_1_FastClock_DIV_PTR, clkDivider);
				BLDCMotor_1_FastClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: BLDCMotor_1_FastClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(BLDCMotor_1_FastClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: BLDCMotor_1_FastClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    BLDCMotor_1_FastClock_MOD_SRC |= modeBitMask & (uint8)BLDCMotor_1_FastClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: BLDCMotor_1_FastClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    BLDCMotor_1_FastClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(BLDCMotor_1_FastClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: BLDCMotor_1_FastClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return BLDCMotor_1_FastClock_MOD_SRC & (uint8)(BLDCMotor_1_FastClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: BLDCMotor_1_FastClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = BLDCMotor_1_FastClock_GetDividerRegister();
    uint8 oldSrc = BLDCMotor_1_FastClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        BLDCMotor_1_FastClock_MOD_SRC |= CYCLK_SSS;
        BLDCMotor_1_FastClock_MOD_SRC =
            (BLDCMotor_1_FastClock_MOD_SRC & (uint8)(~BLDCMotor_1_FastClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: BLDCMotor_1_FastClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return BLDCMotor_1_FastClock_MOD_SRC & BLDCMotor_1_FastClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: BLDCMotor_1_FastClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    BLDCMotor_1_FastClock_PHASE = clkPhase & BLDCMotor_1_FastClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: BLDCMotor_1_FastClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return BLDCMotor_1_FastClock_PHASE & BLDCMotor_1_FastClock_PHASE_MASK;
}
