...
/*******************************************************************************
* Function Name: EncoderClock_2_Start
********************************************************************************
*
* Summary:
...
void EncoderClock_2_Start(void) 
{
    ...
    EncoderClock_2_CLKEN |= EncoderClock_2_CLKEN_MASK;
	EncoderClock_2_CLKSTBY |= EncoderClock_2_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: EncoderClock_2_Stop
********************************************************************************
*
* Summary:
...
void EncoderClock_2_Stop(void) 
{
    ...
    EncoderClock_2_CLKEN &= (uint8)(~EncoderClock_2_CLKEN_MASK);
	EncoderClock_2_CLKSTBY &= (uint8)(~EncoderClock_2_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: EncoderClock_2_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((EncoderClock_2_CLKEN & EncoderClock_2_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = EncoderClock_2_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = EncoderClock_2_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* EncoderClock_2__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(EncoderClock_2_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(EncoderClock_2_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: EncoderClock_2_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: EncoderClock_2_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = EncoderClock_2_GetSourceRegister();
    uint16 oldDivider = EncoderClock_2_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = EncoderClock_2_CLKEN & EncoderClock_2_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(EncoderClock_2_DIV_PTR, clkDivider);
                EncoderClock_2_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                EncoderClock_2_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(EncoderClock_2_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((EncoderClock_2_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: EncoderClock_2_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(EncoderClock_2_DIV_PTR);
}


/*******************************************************************************
* Function Name: EncoderClock_2_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    EncoderClock_2_MOD_SRC |= modeBitMask & (uint8)EncoderClock_2_MODE_MASK;
}


/*******************************************************************************
* Function Name: EncoderClock_2_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    EncoderClock_2_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(EncoderClock_2_MODE_MASK));
}


/*******************************************************************************
* Function Name: EncoderClock_2_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return EncoderClock_2_MOD_SRC & (uint8)(EncoderClock_2_MODE_MASK);
}


/*******************************************************************************
* Function Name: EncoderClock_2_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = EncoderClock_2_GetDividerRegister();
    uint8 oldSrc = EncoderClock_2_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        EncoderClock_2_MOD_SRC |= CYCLK_SSS;
        EncoderClock_2_MOD_SRC =
            (EncoderClock_2_MOD_SRC & (uint8)(~EncoderClock_2_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: EncoderClock_2_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return EncoderClock_2_MOD_SRC & EncoderClock_2_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: EncoderClock_2_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    EncoderClock_2_PHASE = clkPhase & EncoderClock_2_PHASE_MASK;
}


/*******************************************************************************
* Function Name: EncoderClock_2_GetPhase
********************************************************************************
*
* Summary:
...
{
    return EncoderClock_2_PHASE & EncoderClock_2_PHASE_MASK;
}
